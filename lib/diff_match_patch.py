  ...
  """
    ...
    """
        ...
        while pointermin < pointermid:
            if text1[pointerstart:pointermid] == text2[pointerstart:
                                                       pointermid]:
        ...
        while pointer < len(diffs):
            if diffs[pointer][0] == self.DIFF_EQUAL:  # Equality found.
                ...
                if (lastequality and (len(lastequality) <= max(
                        length_insertions1, length_deletions1)) and
                    (len(lastequality) <= max(length_insertions2,
                                              length_deletions2))):
                    # Duplicate record.
            ...
            if diffs[pointer][0] == self.DIFF_INSERT:
                ...
                if count_delete + count_insert > 1:
                    if count_delete != 0 and count_insert != 0:
                        ...
                        commonlength = self.diff_commonPrefix(text_insert,
                                                              text_delete)
                        if commonlength != 0:
                        ...
                        commonlength = self.diff_commonSuffix(text_insert,
                                                              text_delete)
                        if commonlength != 0:
        ...
        while pointer < len(diffs) - 1:
            if (diffs[pointer - 1][0] == self.DIFF_EQUAL and
                ...
                if diffs[pointer][1].endswith(diffs[pointer - 1][1]):
                    ...
                    diffs[pointer] = (
                        diffs[pointer][0], diffs[pointer - 1][1] +
    ...
    """
        ...
        for (op, data) in diffs:
            ...
            if op == self.DIFF_INSERT:
                html.append("<ins style=\"background:#e6ffe6;\">%s</ins>" %
                            text)
            elif op == self.DIFF_DELETE:
                html.append("<del style=\"background:#ffe6e6;\">%s</del>" %
                            text)
    ...
    """
        ...
        # Add the suffix.
        suffix = text[patch.start2 + patch.length1:patch.start2 + patch.length1
                      + padding]
        if suffix:
                    ...
                    if (len(text1) > self.Match_MaxBits and
                        ...
                        for (op, data) in patch.diffs:
                            ...
                            if op == self.DIFF_INSERT:  # Insertion
                                text = text[:start_loc + index2] + data + text[
                                    start_loc + index2:]
                            elif op == self.DIFF_DELETE:  # Deletion
                                text = text[:start_loc + index2] + text[
                                    start_loc + self.diff_xIndex(diffs, index1
                                                                 + len(data)):]
                            if op != self.DIFF_DELETE:
                                index1 += len(data)
            ...
            while len(bigpatch.diffs) != 0:
                ...
                while (len(bigpatch.diffs) != 0 and
                    ...
                    if diff_type == self.DIFF_INSERT:
                        ...
                        empty = False
                    elif (diff_type == self.DIFF_DELETE and
                ...
                if postcontext:
                    ...
                    if len(patch.diffs) != 0 and patch.diffs[-1][
                            0] == self.DIFF_EQUAL:
